...
| M6.4 | Typed execution events and unified event envelope | IN PROGRESS | 3/7 | M6.1, M6.3 | `src/crewspace/dto/events.py` + `tests/test_event_{envelope,ordering_dedupe,resume_cursor}.py`; 73-test bounded gate + independent fail-closed re-review (BLOCKERS: none) | Replay cursor/resume, UI activity render, audit export, Redis/multi-worker seam deferred to later items |
