...
| M6.5 | Approval checkpoints and run-scoped policy | IN PROGRESS | 4/7 | M6.3, M6.4 | `src/crewspace/application/run_policy.py` + `src/crewspace/application/mcp_tools.py` (opt-in checkpoint) + `tests/test_run_policy.py`; 9-test bounded gate | Reuse existing default-deny agent-tool/MCP governance; approval checkpoints record a canonical `approval` event via the M6.4 envelope so they surface in the activity stream without new UI plumbing |
