...
| M6.7 | Agent evaluation and reliability scorecards | IN PROGRESS | 5/7 | M6.3, M6.4 | `src/crewspace/dto/metrics.py` + `src/crewspace/dto/benchmarks.py` (frozen fixture/suite/threshold DTOs) + `src/crewspace/application/metrics.py` (compute_scorecard + compute_team_scorecard) + `src/crewspace/application/benchmarks.py` (materialize_fixture + run_benchmark + compare_cohorts + rank_cohorts + evaluate_regression) + `src/crewspace/infrastructure/repositories.py` (coding_runs.list_for_team) + `tests/test_scorecard.py` + `tests/test_scorecard_team.py` + `tests/test_benchmark_fixtures.py` + `tests/test_benchmark_suite.py` + `tests/test_benchmark_regression.py`; 17-test bounded gate | Replayable benchmarks and version/model comparisons |
...
Acceptance (7/7):
  ...
  - [x] Reconnect resumes from a cursor without gaps or duplicate UI entries.
  - [x] UI renders compact typed activity with raw logs available on demand.
  - [x] Audit JSON/CSV exports include the same canonical events.
...
Acceptance (7/7):
  ...
  - [x] A checkpoint that needs approval emits a canonical `approval` (requested)
        event before the action runs.
  - [x] `granted` approvals allow the action; `denied`/`expired`/`requested`
        (unresolved) fail closed and block execution.
  - [x] Approval decision is tied to principal, run, and action class.
  - [x] Denied/expired/replayed approvals cannot execute the protected action.
  - [x] Every request, decision, and attempted use is recorded as an auditable
        canonical event (surfaces in activity stream + audit export).
  - [x] Security tests cover scope escalation, replay, and stale/expired approvals.
...
Acceptance (7/7):
  ...
  - [x] Reviewer receives independent context plus immutable change-set evidence.
  - [x] Failed/cancelled stages cannot silently advance or duplicate downstream work.
  - [x] Human approval is required before configured delivery actions.
  - [x] UI shows stage status, owner, artifacts, budgets, and blockers.
  - [x] End-to-end real-repo POC reaches a verified human-approved change set.
