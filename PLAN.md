...
| M6.7 | Agent evaluation and reliability scorecards | IN PROGRESS | 6/7 | M6.3, M6.4 | `src/crewspace/dto/metrics.py` + `src/crewspace/dto/benchmarks.py` (frozen fixture/suite/threshold DTOs) + `src/crewspace/application/metrics.py` (compute_scorecard + compute_team_scorecard) + `src/crewspace/application/benchmarks.py` (materialize_fixture + run_benchmark + compare_cohorts + rank_cohorts + evaluate_regression) + `src/crewspace/application/scorecard_view.py` (build_scorecard_view) + `src/crewspace/infrastructure/repositories.py` (coding_runs.list_for_team) + `src/crewspace/templates/scorecard.html` + `tests/test_scorecard*.py` + `tests/test_benchmark_*.py`; 21-test bounded gate | Replayable benchmarks and version/model comparisons |
