...
| M6.8 | Operational inbox | IN PROGRESS | 3/7 | M6.3–M6.5 | `src/crewspace/application/inbox.py` (INBOX_RULES + derive_inbox_id + build_inbox_item + project_inbox_for_team + reconcile_inbox_for_team + load_inbox_for_team) + `tests/test_inbox_projection.py` + `tests/test_inbox_reconcile.py` + `tests/test_inbox_auth.py`; 13-test bounded gate | Human-attention queue across agents, workflows, MCP |
