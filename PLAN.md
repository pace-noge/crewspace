...
| M6.6 | Multi-agent delivery pipeline | IN PROGRESS | 4/7 | M6.2–M6.5 | `src/crewspace/dto/handoffs.py` (contracts + ChangeSetEvidence) + `src/crewspace/application/pipeline.py` (state machine + attach_artifact + duplicate-work guards) + `tests/test_handoff_contracts.py` + `tests/test_pipeline.py` + `tests/test_pipeline_reviewer_evidence.py` + `tests/test_pipeline_no_duplicate_work.py`; 23-test bounded gate | Pass structured handoff artifacts; planner -> coder -> reviewer -> test -> human approval |
