...
| M6.4 | Typed execution events and unified event envelope | IN PROGRESS | 6/7 | M6.1, M6.3 | `src/crewspace/dto/events.py` (EventTransport Protocol + InMemory + Redis adapter) + `tests/test_event_{envelope,ordering_dedupe,resume_cursor,activity,audit_export,transport}.py`; 91-test bounded gate + independent fail-closed re-review (BLOCKERS: none) | Replay cursor/resume, UI activity render, audit export, Redis/multi-worker seam deferred to later items |
